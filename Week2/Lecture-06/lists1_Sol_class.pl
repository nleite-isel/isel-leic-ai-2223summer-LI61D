...
/*
?- reverse(X, mylist(a, mylist(b, empty))).
Z = mylist(b, mylist(a, vazio))

?- reverse(mylist(a, mylist(b, mylist(c, empty))), Z).
Z = mylist(c, mylist(b, lista(a, empty)))
*/
...
reverse3(empty, Z, Z).
reverse3(mylist(C, R), Z, Acc) :- reverse3(R, Z, mylist(C, Acc)).
...
%?- reverse(mylist(1, lista(2, empty)), Z).
%?- reverse(mylist(1, lista(2, lista(a, empty))), Z).
*/
